...
public class OPD_ExistingRegWithOldIC
{
	...
	{	  
		...
		Thread.sleep(5000);
		if(driver.getPageSource().contains("Old IC")) 
		{
			System.out.println("Patient has been selected successfully for primary id Old IC "); 
		} else 
		{
			System.out.println("Patient has been not selected for primary id Old IC"); 
		}


		File screenshot4 = ((TakesScreenshot) driver).getScreenshotAs(OutputType.FILE);
		FileUtils.copyFile(screenshot4, new File(".//ScreenShots//Patient has been selected for primary id Old IC.png"));
